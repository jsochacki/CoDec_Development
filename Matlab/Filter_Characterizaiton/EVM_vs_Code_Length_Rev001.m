...
symbol_pn_sizes = power(2, [2:1:16]);
EVM_dB_vector = zeros(1, length(symbol_pn_sizes));
...
index = 1;
plot(symbol_pn_sizes, EVM_dB_vector,'LineWidth',LW{index},'LineStyle',LS{index},'Marker',MS{index},'Color',LC{index})
%legend_data{index} = sprintf('Chips Per Symbol = %d', symbol_pn_sizes(index));
...
ax.XMinorGrid = 'off';
ax.XMinorTick = 'off';
axis([min(symbol_pn_sizes) max(symbol_pn_sizes) min(EVM_dB_vector) 5])
