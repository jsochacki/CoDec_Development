        ...
        for EbN0 = test_variables.EbNo
            ...
            while ERRORS < ERRORLIMIT
                ...
                filtered_upsampled_code_symbol_stream = ...
                    conv(h, upsampled_code_symbol_stream);

                %Throw away ring up and down
                filtered_upsampled_code_symbol_stream = ...
                    filtered_upsampled_code_symbol_stream(...
                        (1 + ((NSYMBOLS_LONG_FILTER / 2) * USAMPR)) ...
                            : ...
                        (end - ((NSYMBOLS_LONG_FILTER / 2) * USAMPR)));
...
%     axis([min(EbNo_vec) max(EbNo_vec) 1e-2 1e-1])
    ...
    save(sprintf('Results\\BER_Test_Script_Rev012_%d_averages_test_number_%d_Results.mat', ...
         seed, ...
         test_number), ...
         'EbNo_vec', ...
         'BITS_PER_WORD', ...
